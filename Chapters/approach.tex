...
\subsection{Analysis of appearance and spatial coherence}
...
%In practice, we apply complete-link hierarchical agglomerative clustering

%$m$ groups $C = \left \{ c_1, c_2, ..., c_m \right \}$.
%Cluster analysis is the process of classifying objects into subsets that have 
%meaning in the context of a particular problem. Objects are organized into an
%efficient representation that characterizes data. Clustering methods require 
%an index of proximity, or alikeness, be established between pairs or patterns.
%A \textit{proximity matrix} $|d(i, j)|$ accumulates pairwise indices of
%proximity in a matrix, where each row and column represents a pattern. Diagonal
%entries of this matrix are ignored since all patterns are assumed to have same
%degree of proximity with themselves. Also, proximity matrices are symmetric, so  
%all pairs of objects have the same proximity index, independent of the order in
%which they are written.

%A proximity index is either a \textit{similarity} or a \textit{dissimilarity}.
%The more the \textit{i}th and \textit{j}th objects are similar one another,
%the larger a similarity index and the dissimilarity index are. We base our
%approach using the idea of clustering tracking results. On a new frame, each
%tracker will generate a rectangular box of where the object might be. Using this
%information, we are able to create groups of trackers that have similar
%position and scale. For each tracker result, we calculate dissimilarity between
%its bounding box to the rest of trackers running. The dissimilarity $d$ between
%two boxes $\mathrm{x_i}$ and $\mathrm{x_j}$ is computed as:
%\begin{equation}
%d(\mathrm{x_i},\mathrm{x_j}) =
%1 - \frac{\mathrm{x_i} \bigcap \mathrm{x_j}}{\mathrm{x_i} \bigcup  \mathrm{x_j}}
%\text{.}
%\end{equation}

%Using all distances, we construct a symmetric $l \times l$ proximity matrix $D$.
%We take dissimilarities as proximity values. 
%This means that $d(i,i) = 0$ for all $i$.
In practice,
...
%if their dissimilarity 
%to form groups of trackers. Trackers $t_1$ and $t_2$ are \textit{related}
...
\iffalse
CL merges clusters in order of proximity; the closest clusters
will be merged first, and the furthest clusters will be merged last. At each
merge, CL creates a \textit{reduced proximity
matrix}, with one less row and column \cite{Klein2092}. At the end, the
algorithm delivers a set of clusters with size $m$
$C = \left \{ c_1, c_2, ..., c_m \right \}$ satisfying the following:
